...
def main():
	...
	transforms.ToPILImage()(img1.data).save("./data/test1-input1.jpg")
	transforms.ToPILImage()(img2.data).save("./data/test1-input2.jpg")

	print("Images loadad")

	dlk = DeepLK(custom_net(argv[3]))
	print("Model loaded")

	img1 = img1.repeat(5,1,1,1)
	img2 = img2.repeat(5,1,1,1)

	wimg1, p_init_warp = intial_crop_warp(dlk, img1)
	transforms.ToPILImage()(wimg1[0,:,:,:].data).save("./data/test3-input2-warped-initial-p.jpg")

	warped_back_conv = align(dlk, img1, wimg1, p_init_warp)
	transforms.ToPILImage()(warped_back_conv[0,:,:,:].data).save("./data/test4-input2-warped-final.jpg")
...
def intial_crop_warp(dlk, img):
	...
	print("Warping image")
	wimg, _ = warp_hmg(img, H_to_param(dlk.inv_func.apply(param_to_H(pt))))
	print("Image warped")
	return wimg, p
